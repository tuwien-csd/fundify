package at.ac.tuwien.refop.application.service.ris.network;

import at.ac.tuwien.refop.application.port.common.FunderNotFoundException;
import at.ac.tuwien.refop.application.port.common.ProgramNotFoundException;
...
import at.ac.tuwien.refop.application.port.out.persistence.FunderRepository;
import at.ac.tuwien.refop.application.port.out.persistence.ProgramQuery;
import at.ac.tuwien.refop.application.port.out.persistence.ProgramRepository;
import at.ac.tuwien.refop.application.port.out.ris.network.FundingRemoteRepository;
...
public class FundingSynchronizer implements SyncExternalFundingsUseCase {
    ...
    private FunderReference getProgramFunder(Program program) {
        return funderRepository.findReferenceByRisId(program.getFunder().risId()).orElseThrow(FunderNotFoundException::new);
    }

    private FunderReference getCallMainFunder(Call call) {
        return funderRepository.findReferenceByRisId(call.getFunder().risId()).orElseThrow(FunderNotFoundException::new);
    }
    ...
    private ProgramReference getPartOfIfReferenceExists(Call call) {
        ...
        return programQuery.findReference(call.getPartOf().risId()).orElseThrow(ProgramNotFoundException::new);
    }
}
